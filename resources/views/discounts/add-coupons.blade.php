...
$form_action = route('discounts.store-coupons');
$actions = null;

if ($discount->exists) {
	$form_action = route('discounts.update-coupons', $discount->id);
...
$fields_basic = [
	...
	[
		'label' => __('discounts.max-use'),
		'tag' => 'input',
		'attributes' => [
			'id' => 'max_use',
			'name' => 'max_use',
			'type' => 'number',
			'value' => $discount->max_use ?? 0,
			'min' => 0,
			'required' => true
		]
	],
...
@section('content')
	@include('layouts.single_header', ['title' => __('discounts.title-coupon-s'), 'icon' => 'fa fa-edit', 'actions' => $actions, 'updated_at' => $discount->updated_at])
		...
		<div class="card-body">
			...
			<div class="tab-content mt-5">
				<div class="tab-pane fade show active" id="btabs-basic" role="tabpanel">
					@include('layouts.alert', ['icon' => 'fa fa-info', 'state' => 'primary', 'text' => __('discounts.coupon-discount-info')])
